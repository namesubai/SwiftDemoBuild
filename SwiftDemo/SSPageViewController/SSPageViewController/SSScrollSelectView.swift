...
class SSScrollSelectView: UIView {
   ...
   @objc func tapAction(tap:UITapGestureRecognizer) {
    ...
    func selectItem(selectIndex:Int) {
        ...
        self.changeSelectView(selectIndex: selectIndex, finish: {
    ...
    //选择变化处理
    func changeSelectView(selectIndex:Int, finish:() -> Void) {
        for (index,value) in self.labels.enumerated() {
    ...
    func changeSelectView(selectIndex:Int, progress:Float) {
        
        let label = self.labels[selectIndex].label
        
        
//        NSLog("%lf", -(label.frame.width) * CGFloat(progress))
//        NSLog("%lf",progress)
        if progress<0 {
         let tlabel = self.labels[selectIndex-1].label
          self.bottomLineView?.frame = CGRect(x:(tlabel.frame.minX)+(tlabel.frame.width) * CGFloat(abs(progress)), y: label.frame.maxY-CGFloat(bottomLineViewHeight), width: tlabel.frame.width + (label.frame.width-tlabel.frame.width)*CGFloat(abs(progress)), height: CGFloat(bottomLineViewHeight))
        }
        if progress>0 {
           let tlabel = self.labels[selectIndex+1].label
           self.bottomLineView?.frame = CGRect(x:(label.frame.minX)+(label.frame.width) * CGFloat(1-abs(progress)), y: label.frame.maxY-CGFloat(bottomLineViewHeight), width: tlabel.frame.width-(tlabel.frame.width-label.frame.width)*CGFloat(progress), height: CGFloat(bottomLineViewHeight))
        }
    }
